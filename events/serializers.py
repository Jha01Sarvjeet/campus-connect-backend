...
class EventSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if self.instance and 'seats' in data:
            registered_count = self.instance.registrations.filter(status='registered').count()
            if data['seats'] < registered_count:
                raise serializers.ValidationError(
                    {"seats": f"Cannot reduce seats to {data['seats']} since "
                              f"{registered_count} users are already registered."}
                )
